...
// menghapus semua data pakaian lama sesuai id transaksi
mysqli_query($koneksi,"delete from pakaian where pakaian_transaksi='$id'");

// mengambil id transaksi tadi
// $id_transaksi = $id;  <-- Baris ini tampaknya dikomentari di gambar, tapi saya akan sertakan sesuai teks yang ada

// input data pakaian baru (invoice) ke table pakaian
for($x=0;$x<count($jenis_pakaian);$x++){
	if($jenis_pakaian[$x] != ""){
		mysqli_query($koneksi,"insert into pakaian values('','$id_transaksi','$jenis_pakaian[$x]','$jumlah_pakaian[$x]')");
	}
}
